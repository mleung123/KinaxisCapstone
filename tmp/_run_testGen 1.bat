...
if "%GENERATOR_MODEL%"=="" set "GENERATOR_MODEL="
if "%CONTEXT_MODEL%"=="" set "CONTEXT_MODEL=%REVIEW_MODEL%"
if "%REVIEW_MODEL%"=="" set "REVIEW_MODEL=%GENERATOR_MODEL%"
...
:CONFIGURE
echo.
echo Enter a new value, or press Enter to keep the current value shown.
...
set "DEF_GENERATOR_MODEL=%GENERATOR_MODEL%"
set "DEF_CONTEXT_MODEL=%CONTEXT_MODEL%"
...
echo Current: %DEF_DOCKER_CONTAINER%
set /p TMP=New value (or enter to keep): 
...
echo Current: %DEF_LMSTUDIO_LOGPATH%
set /p TMP=New value (or enter to keep): 
...
echo.
set "TMP="
echo Update context model? (rewrites retrieved chunks before generation)
echo Current: %DEF_CONTEXT_MODEL%
set /p TMP=New value (or enter to keep): 
if not "%TMP%"=="" set "CONTEXT_MODEL=%TMP%"
if "%TMP%"=="" set "CONTEXT_MODEL=%DEF_CONTEXT_MODEL%"

echo.
set "TMP="
echo Update generator model? (generates exam items)
echo Current: %DEF_GENERATOR_MODEL%
set /p TMP=New value (or enter to keep): 
if not "%TMP%"=="" set "GENERATOR_MODEL=%TMP%"
if "%TMP%"=="" set "GENERATOR_MODEL=%DEF_GENERATOR_MODEL%"

echo.
set "TMP="
echo Update review model? (evaluates generated items)
echo Current: %DEF_REVIEW_MODEL%
set /p TMP=New value (or enter to keep): 
...
(
  echo @echo off
  echo REM Auto-generated by _run_testGen.bat. Edit if needed.
  echo.
  ...
  echo set "GENERATOR_MODEL=%GENERATOR_MODEL%"
  echo set "CONTEXT_MODEL=%CONTEXT_MODEL%"
...
echo   F = Full pipeline (force re-ingest + generate)
echo   I = Ingest only   (re-ingest, write Chunk Preview XLSX, skip generation)
echo   G = Generate only (use existing DB chunks)
echo.
set "RUN_CHOICE="
set /p RUN_CHOICE=Choice (F / I / G): 

REM Write run_info.txt
echo RUN_ID=%RUN_ID% > "%LOG_DIR%\run_info.txt"
echo UPDATE_CHOICE=%UPDATE_CHOICE% >> "%LOG_DIR%\run_info.txt"
echo RUN_CHOICE=%RUN_CHOICE% >> "%LOG_DIR%\run_info.txt"
...
echo CONTEXT_MODEL=%CONTEXT_MODEL% >> "%LOG_DIR%\run_info.txt"
echo GENERATOR_MODEL=%GENERATOR_MODEL% >> "%LOG_DIR%\run_info.txt"
...
echo CONTEXT_MODEL=%CONTEXT_MODEL%
...
) else if /i "%RUN_CHOICE%"=="I" (
  set "INGEST_ONLY=1"
  python "%RAG_ROOT%\cli.py" pipeline --force-ingest --clear-first --ingest-only 1> "%LOG_DIR%\console_pipeline.txt"
  set "INGEST_ONLY="
) else if /i "%RUN_CHOICE%"=="G" (
